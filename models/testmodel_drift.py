...
class DriftDiffusionModel(nn.Module):
    ...
    def sample_unif(self, x0, no_bridges, t_start, t_end, stepsize, no_samples=100):
        ...
        for k in range(no_bridges):
            ...
            for i in range(disc_steps):
                ...
                vol = sigma * x
                x_new = x + stepsize * drift + torch.sqrt(stepsize) * vol * torch.randn_like(x)
    ...
    def loss(self, batch):
        ...
        taut = self.sigma*(t-t1)*(t2-t)/(t2-t1)+self.rho
        mt = (t2-t)/(t2-t1) * x1 + (t-t1)/(t2-t1) * x2
        x =  mt + torch.sqrt(taut) * torch.randn_like(x1)
        prefactor = self.sigma * ((t1+t2-2*t)/(t2-t1)-1)/(2*taut)
        cond_vel = (x2-x1)/(t2-t1) + prefactor * (x-mt)
