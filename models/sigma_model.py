...
from tgm.utils.network import create_drift_network, create_sigma_network
...
class SigmaModel(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.net = create_sigma_network(model_cfg).to(self.device)
...
class DriftDiffusionModel(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.sigma = SigmaModel(model_cfg).to(self.device)
    ...
    def _loss_drift(self, batch_size, t, t1, t3, x, x1, x3, x_mem, t_mem, mt, prefactor):
        cond_vel = (x3-x1)/(t3-t1) + prefactor * (x-mt)
        
        # predict velocity
        pred_vel = self.forward(x, t, x_mem, t_mem, t3)

        loss = torch.sum((pred_vel-cond_vel.detach())**2)/batch_size 

        return loss

    def _loss_sigma(self, batch_size, pred_sigma, t1, t2, t3, x1, x2, x3):
        lt2 = (t2-t1)*(t3-t2)/(t3-t1)
        mt2 = (t3-t2)/(t3-t1) * x1 + (t2-t1)/(t3-t1) * x3
        cond_sigma = ((x2-mt2)**2-self.rho)/lt2

        loss = torch.sum((pred_sigma-cond_sigma)**2)/batch_size
        #print(cond_sigma)

        return loss, cond_sigma
    ...
    def loss(self, batch):
        ...
        sigma_aux = self.sigma(x = mt, t = t, x_mem = x_mem, t_mem = t_mem, t2 = t3) # Hilfssigma
        #print(sigma_aux.mean())

        taut = sigma_aux*(t-t1)*(t3-t)/(t3-t1)+self.rho
        x =  mt + torch.sqrt(taut) * torch.randn_like(x1)

        prefactor = sigma_aux*((t1+t3-2*t)/(t3-t1)-1)/(2*taut)

        pred_sigma = self.sigma(x, t, x_mem, t_mem, t3) # predicted sigma
        #print(pred_sigma.mean())

        loss_drift = self._loss_drift(batch_size, t, t1, t3, x, x1, x3, x_mem, t_mem, mt, prefactor)
        loss_sigma, cond_sigma = self._loss_sigma(batch_size, pred_sigma, t1, t2, t3, x1, x2, x3)

        loss = loss_drift + loss_sigma
    ...
    def sample_unif(self, x0, no_bridges, t_start, t_end, stepsize, no_samples=100):
        ...
        for k in range(no_bridges):
            ...
            for i in range(disc_steps):
                ...
                #drift = self.mu * x
                sigma = self.sigma(x, t, x_mem, t_mem, t2)
